...
def main(args):
    # Define the output directory and the expected local copy of the EPUB file
    output_parent_dir = os.path.dirname(os.path.abspath(args.output))
    local_epub_path = os.path.join(output_parent_dir, os.path.basename(args.epub_location.filename))

    # Check if the file already exists in the output directory
    if not os.path.exists(local_epub_path):
        # If it doesn't exist, copy the EPUB file to the parent directory of the output file
        shutil.copy(args.epub_location.filename, local_epub_path)
        print(f"Copied EPUB to {local_epub_path}")
    else:
        print(f"Using existing EPUB file at {local_epub_path}")

    # Open the copied or existing EPUB file
    epub_zip = zipfile.ZipFile(local_epub_path)
...
# 
    ...
    with open(args.output, "w", encoding="utf-8") as outfile:
        json.dump(general_book_dictionary, outfile, indent=4, sort_keys=True, ensure_ascii=False)
